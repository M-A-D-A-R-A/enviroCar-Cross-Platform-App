...
// TODO: uncomment the following import to parse the data received from the OBD-II adapter
...
class BluetoothProvider extends ChangeNotifier {
  ...
  void addDeviceToList(BluetoothDevice bluetoothDevice) {
    // TODO: add only OBD-II devices to the [_detectedBluetoothDevices] list
  ...
  void mapCharacteristicsToService() {
    for (final BluetoothService service in _services) {
      ...
      for (final BluetoothCharacteristic characteristic
        ...
        // the service uuid having "fff0" as substring and the characteristic starting with "0000fff1-0000-1000-8000" might be used for OBD response parsing
        if (service.uuid.toString().toLowerCase().substring(4, 8) == "fff0") {
          if (characteristic.uuid
              .toString()
              .startsWith("0000fff1-0000-1000-8000", 0)) {
  ...
  Future interactWithDevice(List<int> value) async {
    // TODO: interact with the OBD-II device and retrieve data from the characteristics to parse the data for determining the parameters
    ...
    if (notifyCharacteristicsQueue.isNotEmpty) {
      for (final BluetoothCharacteristic notifyCh
          in notifyCharacteristicsQueue) {
  ...
  void dataCallback(List<int> value) {
    ...
    // TODO: to use the following functions uncomment the obdResponseParser import
  ...
  Future<List<int>> readCharacteristics(
    ...
    if (readValue != null) {
      debugPrint(
        'read characteristic ${characteristic.uuid.toString()} its value is ${readValue.toString()}',
      );
      return readValue;
    }
  ...
  Future writeCharacteristics(
    ...
    debugPrint(
      'response after writing the characteristics ${characteristic.uuid.toString()} is $response',
    );
  }
  ...
  Future notify(BluetoothCharacteristic characteristic) async {
    ...
    characteristic.value.listen((value) {
      debugPrint(
        'characteristics (${characteristic.uuid.toString()}) value updated, its value is ${value.toString()}',
      );
    });
  }
